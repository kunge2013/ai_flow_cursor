...
public class DeepSeekAdapter implements AiModelAdapter {
    ...
    public String testConnection(ModelTestDTO testDTO) {
        try {
            ...
            if (response.statusCode() == 200) {
                log.info("DeepSeek连接测试成功: {}", response.body());
                return "连接成功！测试响应: " + response.body();
            } else {
                log.error("DeepSeek连接测试失败，状态码: {}", response.statusCode());
    ...
    public String generateText(ModelDTO modelDTO, String prompt, Integer maxTokens, Double temperature) {
        try {
            ...
            if (response.statusCode() == 200) {
                log.info("DeepSeek文本生成成功: {}", response.body());
                return response.body();
            } else {
                throw new RuntimeException("DeepSeek文本生成失败，状态码: " + response.statusCode() + ", 响应: " + response.body());
            }
        } catch (Exception e) {
            log.error("DeepSeek文本生成失败", e);
            throw new RuntimeException("DeepSeek文本生成失败: " + e.getMessage(), e);
        }
    }
